<div
    ...
    @if ($appointment_unfulfilled_notification)
        <div id="appointment_unfulfilled"
            ...
            <button wire:click="clear_appointment_unfulfilled"
                class="text-white border-2 border-white px-4 rounded-lg mt-2 hover:scale-110 transition-all">Close</button>
...
transition-all {{ count($all_appointments) == 0 ? 'hidden' : '' }}  shadow-[0_4px_4px_0_rgba(0,0,0,0.25)]"
    ...
    <div class="mt-4 flex flex-col gap-4">
        ...
        @foreach ($all_appointments as $appointment)
            <div
                class="flex flex-col justify-center  items-center w-[96vw]  md:max-w-[800px]  md:p-8 px-4 py-8  mx-auto mt-2 rounded-lg {{ session('theme_mode') == 'light' ? 'bg-[#d6e0ec]' : 'bg-[#1e1d1d]' }}  shadow-[0_4px_4px_0_rgba(0,0,0,0.25)] {{ in_array($appointment['booked_client_id'], $appointment_unfulfilled_selected_id) || in_array($appointment['booked_client_id'], $appointment_restored_selected_id) ? 'hidden' : '' }}">
                ...
                <p
                    ...
                    <b>Estimated
                        Fee:&nbsp;</b>{{ $appointment['estimated_price'] ? $appointment['estimated_price'] : 'Not Available' }}
                </p>
 ...
 transition-all shadow-[0_4px_4px_0_rgba(0,0,0,0.25)]">Restore
                    ...
                    <button wire:click="markAsUnfulfilled({{ $appointment['booked_client_id'] }})"
                        class="px-4 py-2 w-[200px] bg-red-800 text-white rounded-lg hover:scale-110 transition-all  shadow-[0_4px_4px_0_rgba(0,0,0,0.25)]">Mark
                        As Unfulfilled</button>
