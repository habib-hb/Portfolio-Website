...
const css_stablizer = () => {
    ...
    setTimeout(() => {
        ...
        if (
            ...
            document.activeElement !== document.getElementById("search_input")
        ) {
...
document.addEventListener("livewire:initialized", () => {
    ...
    Livewire.on("no_results_found", () => {
        // Doing 100ms delay cause the DOM is not loaded yet.
        setTimeout(() => {
            document.getElementById("no_results_found").textContent =
                "No Results Found";

            if (
                document
                    .getElementById("no_results_found")
    ...
    Livewire.on("load_animation", () => {
        // Doing this to avoid the rerendering issue in javascript
        animation_function();
    });
    ...
    document.addEventListener("click", function (event) {
        ...
        const notify_error_element =
            document.getElementById("notify_error_element") || null;
        const isClickInside = admin_login_popup.contains(event.target);
        const isClickInside_notify_error_element =
            notify_error_element?.contains(event.target);

        // If the click is outside the dropdown, perform an action
        if (!isClickInside && !isClickInside_notify_error_element) {
            if (
                !document
                    .getElementById("admin_login_popup")
                    .classList.contains("hidden")
            ) {
